---
title: In-Depth Study of Offline Connector
date: 2026-05-28
topics: [vllm]
---
...
2. Request (请求id，token数据，block_hashes。。。)
3. group_states
    1. offload_keys (block_hash + group_idx)
    2. block_ids scheduler分配的
    3. next_stored_block_idx 跟踪 offload 进度（哪个 block 是下一个要保存的

    4. num_hit_blocks 跟踪进行中的传输 jobs（load 或 store）

### TransferJobStatus

1. req_id: ReqId                         # 属于哪个请求
2. pending_count: int                    # 还有几个 worker 没完成
3. keys: set[OffloadKey]                 # 这个 job 涉及的 offload keys
4. is_store: bool                        # True=store(GPU→CPU), False=load5. (CPU→GPU)
5. non_sliding_window_block_ids: list[int] | None = None   # 非滑动窗口 block IDs
6. sliding_window_block_ids: list[int] | None = None       # 滑动窗口 block IDs
...
### 始化：
通过config创建 OffloadingSpec, 然后创建两个role，connector_scheduler，connector_worker

### get_num_new_matched_tokens：

1. 查找group_states或者创建空group_states
2. update_offload_keys: 增量补充offload_key，根据block_hashes
3. lookup: 其实就是遍历了内存，用了链表计数器决定哪些值得offload，LRU作为实际的block，避免一些低频率的数据作为offload
4. touch：就是动了一下链表

### bind_gpu_block_pool

1. 保存了个VRAM的页表BlockPool

关键点：VRAM和内存都是基于BlockPool，这是一个python实现的页表管理
...
2. 判断需要加载哪些block
3. PD分离需要把整个都offload
4. 检查冲突，读写冲突的话需要让worker阻塞，需要有想关的判断
5. prepare_store更新一下LRU，然后返回CPULoadStoreSpec，包含了block id信息
6. 生成TransferJob
7. _blocks_being_loaded降低并发

### build_connector_meta

1. 把抢占，已经完成的任务加入_current_batch_jobs_to_flush
...
```
manager.prepare_store([K0]) # 分配cpu block
...
1. 这些 jobs 对应的请求状态正在被清理 (因为被抢占或完成)，worker 必须尽快回报完成状态，这样 scheduler 才能安全清理。